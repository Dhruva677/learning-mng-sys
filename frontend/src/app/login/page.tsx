...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setError(msg?.error ?? msg?.errors?.[0]?.msg ?? 'Login failed. Check your credentials.');
    }
  };
    ...
    <div className="min-h-[calc(100vh-3.5rem)] flex items-center justify-center px-4 bg-gray-50">
      <div className="w-full max-w-md bg-white rounded-xl border border-gray-200 p-8 shadow-sm">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="input-field" required autoComplete="current-password" placeholder="••••••••"
            />
          </div>
        ...
        <div className="mt-4 p-3 bg-gray-50 rounded-lg text-xs text-gray-400 text-center">
          Demo account: <span className="font-mono text-gray-600">demo@example.com</span> / <span className="font-mono text-gray-600">password123</span>
        </div>
      </div>
    </div>
  );
